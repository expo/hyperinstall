...
class Hyperinstall {
      ...
      if (state.cacheBreaker !== CACHE_BREAKER) {
        yield Promise.all((0, _map2.default)(packages, (() => {
          var ref = _asyncToGenerator(function* (cacheBreaker, name) {
            let deps = yield _this.readPackageDepsAsync(name);
            let unversionedDepChecksums = yield _this.readUnversionedDepChecksumsAsync(name, deps);
            yield _this.updatePackageAsync(name, cacheBreaker, deps, unversionedDepChecksums);
          });
  ...
  updatePackageIfNeededAsync(name, cacheBreaker) {
    ...
    return _asyncToGenerator(function* () {
      let deps = yield _this5.readPackageDepsAsync(name);
      let unversionedDepChecksums = yield _this5.readUnversionedDepChecksumsAsync(name, deps);
      if (_this5.forceInstallation) {
        yield _this5.removeNodeModulesDirAsync(name);
        yield _this5.updatePackageAsync(name, cacheBreaker, deps, unversionedDepChecksums);
      } else if (_this5.packageNeedsUpdate(name, cacheBreaker, deps, unversionedDepChecksums)) {
        yield _this5.updatePackageAsync(name, cacheBreaker, deps, unversionedDepChecksums);
      }
    })();
  }

  updatePackageAsync(name, cacheBreaker, deps, unversionedDepChecksums) {
    var _this6 = this;

    return _asyncToGenerator(function* () {
      let packagePath = _path2.default.resolve(_this6.root, name);
      yield _this6.installLock.acquireAsync();
      console.log('Package "%s" has been updated; installing...', name);
      try {
        yield _this6.execNpmInstallAsync(packagePath);
        console.log('Finished installing "%s"\n', name);
      } finally {
        _this6.installLock.release();
      }

      _this6.updatedPackages[name] = {
        cacheBreaker,
        dependencies: deps,
  ...
  removeNodeModulesDirAsync(name) {
    ...
    return _asyncToGenerator(function* () {
      let nodeModulesPath = _path2.default.resolve(_this7.root, name, 'node_modules');
  ...
  readPackageDepsAsync(name) {
    ...
    return _asyncToGenerator(function* () {
      let packageJSONPath = _path2.default.resolve(_this8.root, name, 'package.json');
  ...
  readUnversionedDepChecksumsAsync(name, deps) {
    ...
    return _asyncToGenerator(function* () {
      let packagePath = _path2.default.resolve(_this9.root, name);
      let unversionedDeps = _this9.filterLocalDeps(name, deps);
      let promises = {};
      for (let _ref3 of (0, _toPairsIn2.default)(unversionedDeps)) {
        ...
        promises[dep] = _this9.readPackageChecksumAsync(absoluteDepPath);
      }
  ...
  filterLocalDeps(name, deps) {
    ...
    let localDeps = {};
    try {
      for (let _ref5 of (0, _toPairsIn2.default)(deps)) {
    ...
    return _asyncToGenerator(function* () {
      return new Promise(function (resolve, reject) {
        ...
        fileListStream.on('child', function (entry) {
          ...
          fileChecksumPromises[relativeFilePath] = _this10.readFileChecksumAsync(absoluteFilePath, 'sha1');
        });
  ...
  cleanAsync() {
    ...
    return _asyncToGenerator(function* () {
      let stateFilename = _path2.default.join(_this11.root, STATE_FILE);
