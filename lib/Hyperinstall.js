...
class Hyperinstall {
  ...
  readTargetPackageStateAsync(name) {
    ...
    return _asyncToGenerator(function* () {
      let [deps, yarnLockfile, shrinkwrap] = yield Promise.all([_this5.readPackageDepsAsync(name), _this5.readYarnLockfileAsync(name), _this5.readShrinkwrapAsync(name)]);
    ...
    return _asyncToGenerator(function* () {
      ...
      try {
        ...
        if (targetPackageState.yarnLockfile) {
          yield (0, _yarn.execYarnInstallAsync)(packagePath);
        } else {
  ...
  readYarnLockfileAsync(name) {
    var _this9 = this;

    return _asyncToGenerator(function* () {
      let lockfilePath = _path2.default.resolve(_this9.root, name, 'yarn.lock');
      let lockfile;
      try {
        lockfile = yield _fs2.default.promise.readFile(lockfilePath, 'utf8');
      } catch (e) {
        if (e.code === 'ENOENT') {
          return undefined;
        }
  ...
  readShrinkwrapAsync(name) {
    ...
    return _asyncToGenerator(function* () {
      let shrinkwrapJSONPath = _path2.default.resolve(_this10.root, name, 'npm-shrinkwrap.json');
      let shrinkwrapJSON;
      try {
  ...
  readPackageDepsAsync(name) {
    ...
    return _asyncToGenerator(function* () {
      let packageJSONPath = _path2.default.resolve(_this11.root, name, 'package.json');
  ...
  readUnversionedDepChecksumsAsync(name, deps) {
    ...
    return _asyncToGenerator(function* () {
      let packagePath = _path2.default.resolve(_this12.root, name);
      let unversionedDeps = _this12.filterLocalDeps(name, deps);
      let promises = {};
      for (let [dep, depPath] of (0, _toPairsIn2.default)(unversionedDeps)) {
        ...
        promises[dep] = _this12.readPackageChecksumAsync(absoluteDepPath);
      }
    ...
    return _asyncToGenerator(function* () {
      return new Promise(function (resolve, reject) {
        ...
        fileListStream.on('child', function (entry) {
          ...
          fileChecksumPromises[relativeFilePath] = _this13.readFileChecksumAsync(absoluteFilePath, 'sha1');
        });
  ...
  packageNeedsUpdateAsync(name, cacheBreaker, targetPackageState) {
    ...
    return _asyncToGenerator(function* () {
      let packageState = (0, _get2.default)(_this14.state.packages, name);
      ...
      if (targetYarnLockfile) {
        return !(0, _isEqual2.default)(targetYarnLockfile, installedYarnLockfile);
      }
      ...
      if (!(0, _isEmpty2.default)(installedDeps)) {
        let nodeModulesPath = _path2.default.resolve(_this14.root, name, 'node_modules');
        let isNodeModulesPresent = yield _this14.isDirectoryAsync(nodeModulesPath);
  ...
  cleanAsync() {
    ...
    return _asyncToGenerator(function* () {
      let stateFilename = _path2.default.join(_this15.root, STATE_FILE);
